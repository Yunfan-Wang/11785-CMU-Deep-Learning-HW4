...
class ASRDataset(Dataset):
    def __init__(
        ...
        if subset_size < 1.0:
            cutoff = int(len(self.fbank_files) * subset_size)
            self.fbank_files = self.fbank_files[:cutoff]
        ...
        if self.partition != "test-clean":
            ...
            # TODO: Take subset
            if subset_size < 1.0:
                self.text_files = self.text_files[:cutoff]
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                transcript = np.load(text_path).tolist()
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        padded_feats = pad_sequence(batch_feats, batch_first=True) # B x T x F
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.LongTensor([seq.shape[0] for seq in batch_shifted])
